...
#if defined(NDEBUG)
# error "Articfox cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x83356a83b926264ddaaedd791f5f166f376b32abd35d46f65989197c7c28241f");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Articfox: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Articfox Signed Message:\n";
...
{
    // Articfox: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 210000); // Articfox: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Articfox: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Articfox: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Articfox: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Articfox: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x34d4660fd85f2bef17049568ffa49d0b08bc6719c708be88f44638f96737d810");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "15/05/2021 - ArticFox Coin Killed Dogecoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 250000 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04df7a0035cc505e6d97f29f1732f481e5cf2b61b878d021fc8e24e6b8fe5defe87cd6c81d2ab1b5ad6f4bdd53a607c4fc7d0b421326b308c5f3271eecd3e770ed") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1621074655;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086897595;

        if (fTestNet)
        {
            block.nTime    = 1621074626;
            block.nNonce   = 390462325;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x1ca2784009885d40dece9e2e1ee321905cd1485892efdb1dddc71fb7663df75d"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc2, 0xb4, 0xdc }; // Articfox: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ArticfoxMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ArticfoxMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ArticfoxMiner(CWallet *pwallet)
{
    printf("ArticfoxMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("articfox-miner");
    ...
    try { loop {
        ...
        printf("Running ArticfoxMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ArticfoxMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ArticfoxMiner, pwallet));
}
